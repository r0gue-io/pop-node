...
impl<T: Config> Default for EraInfo<T> {
	fn default() -> Self {
		Self {
			next_era_start: 1,
			contract_fee_total: MainBalanceOf::<T>::zero(),
			total_fee_amount: MainBalanceOf::<T>::zero(),
		}
	}
}
