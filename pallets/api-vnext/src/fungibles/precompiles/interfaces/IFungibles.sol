...
interface IFungibles {
	 ...
	 */
    function create(address admin, uint256 minBalance) external returns (uint32 id);
    /**
     * @dev Set the metadata for a token.
	 */
    function setMetadata(uint32 id, string name, string symbol, uint8 decimals) external;
    /**
	 ...
	 */
    function transferFrom(uint32 id, address from, address to, uint256 value) external;
    /**
	 ...
	 */
    event Transfer(uint32 indexed id, address indexed from, address indexed to, uint256 value);
}