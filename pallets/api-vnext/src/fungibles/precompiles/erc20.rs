	...
	fn call(
		...
		match input {
			...
			totalSupply(_) => {
				...
				let total_supply = U256::saturating_from(<Assets<T, I>>::total_supply(id));
			...
			balanceOf(balanceOfCall { account }) => {
				...
				let balance = U256::saturating_from(<Assets<T, I>>::balance(id, account));
			...
			allowance(allowanceCall { owner, spender }) => {
				...
				let remaining =
					U256::saturating_from(<Assets<T, I>>::allowance(id, &owner, &spender));
			...
			symbol(_) => {
				// TODO: charge based on benchmarked weight
				let result = <Assets<T, I>>::symbol(id);
				// TODO: improve
				let result = String::from_utf8_lossy(result.as_slice());
			...
			decimals(_) => {
				// TODO: charge based on benchmarked weight
				let result = <Assets<T, I>>::decimals(id);
...
mod tests {
	...
	use super::*;
	use crate::mock::{ExtBuilder, RuntimeOrigin};

	const ERC20: u16 = 101;

	type Assets = mock::Assets;
	...
	fn total_supply_works() {
		...
		ExtBuilder::new()
			...
			.with_asset_balances(vec![(id, ALICE, total_supply)])
	...
	fn transfer_works() {
		...
		let endowment = 10_000_000;
		ExtBuilder::new()
			.with_assets(vec![(id, BOB, true, 1)])
			.with_asset_balances(vec![(id, BOB, endowment)])
			.build_with_env(|mut call_setup| {
				call_setup.set_origin(Origin::Signed(BOB));
				assert_eq!(Assets::balance(id, BOB), endowment);
				...
				assert_eq!(Assets::balance(id, BOB), endowment - value);
				...
				assert_last_event(prefixed_address(ERC20, id), event);
			});
	}
	...
	fn name_works() {
		...
		ExtBuilder::new()
			.with_assets(vec![(id, ALICE, false, 1)])
			.with_asset_metadata(vec![(id, _name.as_bytes().to_vec(), b"symbol".to_vec(), 10)])
			.build_with_env(|mut call_setup| {
				assert_eq!(
					call_precompile::<String>(&mut call_setup.ext().0, id, &name(nameCall {}))
						.unwrap()
						.as_str(),
					_name
				);
	...
	fn symbol_works() {
		...
		ExtBuilder::new()
			.with_assets(vec![(id, ALICE, false, 1)])
			.with_asset_metadata(vec![(id, b"name".to_vec(), _symbol.as_bytes().to_vec(), 10)])
			.build_with_env(|mut call_setup| {
				assert_eq!(
					call_precompile::<String>(&mut call_setup.ext().0, id, &symbol(symbolCall {}))
						.unwrap()
						.as_str(),
					_symbol
				);
	...
	fn decimals_works() {
		...
		ExtBuilder::new()
			.with_assets(vec![(id, ALICE, false, 1)])
			.with_asset_metadata(vec![(id, b"name".to_vec(), b"symbol".to_vec(), _decimals)])
			.build_with_env(|mut call_setup| {
				let mut ext = call_setup.ext().0;
				assert_eq!(
					call_precompile::<u16>(&mut ext, id, &decimals(decimalsCall {})).unwrap() as u8,
					_decimals
				);
			});
	}
	...
	fn call_precompile<Output: SolValue + From<<Output::SolType as SolType>::RustType>>(
		...
		super::call_precompile::<Erc20, Output>(ext, &prefixed_address(ERC20, id), input)
	}
}
