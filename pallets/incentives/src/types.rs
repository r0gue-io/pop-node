...
pub struct EraInfo<T: Config> {
	...
	pub(crate) next_era_start: BlockNumberFor<T>,
...
impl<T: Config> EraInfo<T> {
	/// Block number at which the next era should start.
	pub fn next_era_start(&self) -> BlockNumberFor<T> {
		self.next_era_start
	}

	/// Set the next era start block number.
	pub fn set_next_era_start(&mut self, next_era_start: BlockNumberFor<T>) {
		self.next_era_start = next_era_start;
	}

	/// Checks whether a new era should be triggered.
	pub fn is_new_era(&self, now: BlockNumberFor<T>) -> bool {
