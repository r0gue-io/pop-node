...
pub mod pallet {
	...
	pub mod config_preludes {
		...
		impl Time for TestDefaultConfig {
			...
			fn now() -> Self::Moment {
				unimplemented!(
					"No default `now` implementation in `TestDefaultConfig` provide a custom \
					 `T::Time` type."
				)
			}
		}
	...
	T::Hash: frame_support::traits::IsType<H256>,
{
		...
		let extract_error = |err| {
				...
				err == Error::<T>::StorageDepositLimitExhausted.into()
			{
				...
				return Err(EthTransactError::Message(format!(
					"insufficient funds for gas * price + value: address {from:?} have {balance} \
					 (supplied gas {})",
	...
	fn run_guarded<R, F: FnOnce() -> Result<R, ExecError>>(f: F) -> Result<R, ExecError> {
		executing_contract::using_once(&mut false, || {
			...
			.expect(
				"Returns `Ok` if called within `using_once`. It is syntactically obvious that \
