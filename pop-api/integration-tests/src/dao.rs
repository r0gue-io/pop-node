...
use super::*;
use crate::{messaging::{IsmpGetCompleted, ISMP_MODULE_ID, get_ismp_request}, bare_call, function_selector, instantiate, upload, INIT_VALUE};
...
fn register() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(pallet_nfts::Account::<Runtime>::get((ALICE, collection, 1)), Some(()));
		assert_eq!(ALICE, pallet_nfts::Pallet::<Runtime>::owner(collection, 1).unwrap());
	});
}
...
impl Dao {
	fn new() -> Self {
		...
		let (address, account_id) = instantiate(DAO, INIT_VALUE, function, vec![]);
