...
use pop_runtime_devnet::{Balances, Sponsorships};
...
impl Contract {
    ...
    pub fn withdraw_sponsorship(&self, beneficiary: &AccountId32) -> Result<(), Error> {
        let result = self.call_with_signer("withdraw_sponsorship", Default::default(), 0, beneficiary.clone());
        <Result<(), Error>>::decode(&mut &result.data[1..])
            .unwrap_or_else(|_| panic!("Contract reverted: {:?}", result))
    }
    ...
    fn call_with_signer(&self, function: &str, params: Vec<u8>, value: u128, signer: AccountId32) -> ExecReturnValue {
        let function = function_selector(function);
        let params = [function, params].concat();
