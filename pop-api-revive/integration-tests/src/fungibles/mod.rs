...
fn total_supply_works() {
	new_test_ext().execute_with(|| {
		...
		log::info!("TOTAL_SUPPLY2");
		assert_eq!(total_supply(&addr, TOKEN_ID), Ok(0));

		// Tokens in circulation.
		log::info!("Creating and minting to Bob");
...
fn instantiate_and_create_fungible_works() {
	new_test_ext().execute_with(|| {
		...
		log::info!("Instantiator: {:?}", instantiator);
		log::info!("Asset owner: {:?}", Assets::owner(TOKEN_ID));
